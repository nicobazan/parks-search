...
async function populateParks(httpClient: HttpClient, current: number) {

    const { limit, total, data } = await <Promise<{ limit: number, start: number, total: number, data: Park[] }>>getParks(httpClient, current);
...
export async function getParks(httpClient: HttpClient, start?: number, limit?: number) {
